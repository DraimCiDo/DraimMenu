...
public class GetCustomHeads {
    DraimMenu plugin;
    public GetCustomHeads(DraimMenu pl) {
        this.plugin = pl;
    }
    ...
    public ItemStack getCustomHead(String b64stringtexture) {
        ...
        if (propertyMap == null) {
            throw new IllegalStateException("Профиль не содержит карты свойств");
        } else {
    ...
    private <T> Field getField(Class<?> target, String name, Class<T> fieldType, int index) {
        ...
        if (target.getSuperclass() != null) {
            ...
            throw new IllegalArgumentException("Не удается найти поле с типом " + fieldType);
        }
    }
}
